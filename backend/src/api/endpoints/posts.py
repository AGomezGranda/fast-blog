...
from backend.src.controller.post_controller import get_all_posts, get_post_by_uuid, create_post, update_post, delete_post
from backend.src.controller.user_controller import get_current_user
...
#get post by uuid
@router.get("/{uuid}", response_model=Post)
...
#update post by uuid
@router.put("/{uuid}", response_model=Post)
def update_post(post_uuid: str, post_in: PostUpdate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    post = get_post_by_uuid(db, post_uuid)
    if not post:
        raise HTTPException(status_code=404, detail="Post not found")
